...
import com.gemini.ambacam.repository.RoleRepository;
...
public class RequerantsResourceIT extends ItBase {
    ...
    @Test
    public void createCreatorNotFound() {
    ...
    public void list() {
        given().get(ApiConstants.REQUERANT_COLLECTION).then().log().body().statusCode(200)
                ...
                .body("find{it.id==" + requerant1.getId().intValue() + "}.profession", is(equalTo(requerant1.getProfession())))
    ...
    private RequerantCreateTO buildRequerantCreateTO(Long creatorId, Long paysId) {
        return new RequerantCreateTO()
                .nom("nom-" + UUID.randomUUID())
                .prenom("prenom-" + UUID.randomUUID())
                .dateNaissance(new Date())
                .creatorId(creatorId)
                .sexe("sexe-" + UUID.randomUUID())
                .profession("profession-" + UUID.randomUUID())
                .lieuNaissance("lieuNaissance-" + UUID.randomUUID())
                .paysId(paysId);
    }

    private RequerantUpdateTO buildRequerantUpdateTO(Long paysId) {
        return new RequerantUpdateTO()
                .nom("nom-" + UUID.randomUUID())
                .prenom("prenom-" + UUID.randomUUID())
                .dateNaissance(new Date())
                .sexe("sexe-" + UUID.randomUUID())
