...
public class PayssResourceIT extends ItBase {
	...
	public void create() {
		...
		int id = given().contentType(ContentType.JSON).body(create).log().body().post(ApiConstants.PAYS_COLLECTION)
	...
	public void createNomNull() {
		Pays create = buildPays();
		create.setNom(null);
		given().contentType(ContentType.JSON).body(create).log().body().post(ApiConstants.PAYS_COLLECTION).then().log()
				.body().statusCode(400);
	}
	...
	@Test
	public void list() {
		given().get(ApiConstants.PAYS_COLLECTION).then().log().body().statusCode(200).body("size()", is(equalTo(2)))
	...
	@Test
	public void get() {
		given().get(ApiConstants.PAYS_ITEM, pays2.getId()).then().log().body().statusCode(200)
	...
	public void getNotFound() {
		given().get(ApiConstants.PAYS_ITEM, random.nextLong()).then().statusCode(404);
	}

	@Test
	public void delete() {
		given().delete(ApiConstants.PAYS_ITEM, pays1.getId()).then().statusCode(200);
	...
	public void deleteNotFound() {
		given().delete(ApiConstants.PAYS_ITEM, random.nextLong()).then().log().body().statusCode(404);
	}

	@Test
	public void update() {
		Pays update = buildPays();
		given().contentType(ContentType.JSON).body(update).put(ApiConstants.PAYS_ITEM, pays2.getId()).then().log()
				.body().statusCode(200);
	...
	public void updateNotFound() {
		Pays update = buildPays();
		given().contentType(ContentType.JSON).body(update).put(ApiConstants.PAYS_ITEM, random.nextLong()).then().log()
				.body().statusCode(404);
	}
	...
	public void updateMemeNom() {
		...
		given().contentType(ContentType.JSON).body(update).log().body().put(ApiConstants.PAYS_ITEM, pays1.getId())
