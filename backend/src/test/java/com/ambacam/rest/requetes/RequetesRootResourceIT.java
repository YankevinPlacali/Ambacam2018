...
public class RequetesRootResourceIT extends ItBase {
	...
	public void listRequerantByDateNaissanceAndIdentifier() {
		...
		// SUT
		given().contentType(ContentType.JSON).body(identifyRequerantTO).log().body()
				.post(ApiConstants.REQUETE_HISTORY_COLLECTION).then().log().body().statusCode(200)
	...
	public void listRequerantByDateNaissanceAndIdentifierFilure() {
		...
		given().contentType(ContentType.JSON).body(identifyRequerantTO).log().body()
				.post(ApiConstants.REQUETE_HISTORY_COLLECTION).then().log().body().statusCode(400);
	}
